		...
		while(count>0){
			for(y=0;y<size.getY();y++){
				for(x =0; x<size.getX(); x++){
					if(matrix[x][y].retId()==type && matrix[x][y].retCoord().retZ() == z && count>0){ //the previous instance must be the same type and the same height, count needs to be greater than 0
